...
describe('fetchRSS', function() {
    ...
    it('解析RSS並轉為統一格式, from優先取creator否則退回訂閱源標題', async function() {
        let r = await fetchRSS(svr.url('/rss/feed.xml'), { useShowLog: false })
        let rr = [
            {
    ...
    it('HTTP 404納入重試範圍, 含初始共執行maxRetries+1次', async function() {
        ...
        await fetchRSS(svr.url('/status/404'), { maxRetries: 2, baseDelayMs: 10, maxDelayMs: 10, useShowLog: false })
            .catch(() => {})
    ...
    it('內容非XML時reject', async function() {
        let r = null
        await fetchRSS(svr.url('/json/ok'), { maxRetries: 0, useShowLog: false })
            .then(() => {
                r = 'resolved'
            })
            .catch(() => {
    ...
    it('預設不附content欄, 回傳形狀維持五欄不變', async function() {
        let t = await fetchRSS(svr.url('/rss/feed-content.xml'), { useShowLog: false })
    ...
    it('withContent為true時附content欄, 取content:encoded去標籤與實體且顯著長於description', async function() {
        let t = await fetchRSS(svr.url('/rss/feed-content.xml'), { withContent: true, useShowLog: false })
        let r = [
            Object.keys(t[0]),
            t[0].description,
            t[0].content,
    ...
    it('withContent對無content:encoded之feed(arXiv系)退取content, 全文與description一致', async function() {
        let t = await fetchRSS(svr.url('/rss/feed-desc.xml'), { withContent: true, useShowLog: false })
    ...
    it('method為curl時走系統curl抓取, 解析結果與fetch相同', async function() {
        let t = await fetchRSS(svr.url('/rss/feed.xml'), { method: 'curl', maxRetries: 0, useShowLog: false })
    ...
    it('curl模式對極短feed判empty-response而reject, 證明分派確實走curl路徑', async function() {
        ...
        let r = null
        await fetchRSS(svr.url('/rss/feed-tiny.xml'), { method: 'curl', maxRetries: 0, useShowLog: false })
            .then(() => {
                r = 'resolved'
            })
            .catch((err) => {
    ...
    it('fetch模式對極短feed正常解析為0項, 與curl模式行為對照', async function() {
        let r = await fetchRSS(svr.url('/rss/feed-tiny.xml'), { maxRetries: 0, useShowLog: false })
        let rr = []
        assert.strict.deepEqual(r, rr)
    })

    it('auto模式於curl失敗時退回fetch, 極短feed不reject而回0項', async function() {
        let r = await fetchRSS(svr.url('/rss/feed-tiny.xml'), { method: 'auto', maxRetries: 0, useShowLog: false })
        let rr = []
        assert.strict.deepEqual(r, rr)
    })
    ...
    it('method非法值一律退回預設fetch行為', async function() {
        ...
        for (let method of ['FETCH', 'wget', 123, null, {}]) {
            let t = await fetchRSS(svr.url('/rss/feed.xml'), { method, useShowLog: false })
            r.push(t.length)
        }
        let rr = [2, 2, 2, 2, 2]
        assert.strict.deepEqual(r, rr)
