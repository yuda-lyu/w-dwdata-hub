...
describe('fetchCnyes', function() {
    ...
    it('轉為統一格式並組出新聞內容頁網址', async function() {
        let r = await fetchCnyes({ url: svr.url('/cnyes/newslist'), pageDelayMs: 0, useShowLog: false })
        let rr = [
    ...
    it('publishAt缺漏時該筆time退回空字串, 不影響其餘新聞', async function() {
        let t = await fetchCnyes({ url: svr.url('/cnyes/newslist'), pageDelayMs: 0, useShowLog: false })
    ...
    it('無更多資料時停止翻頁', async function() {
        ...
        await fetchCnyes({ url: svr.url('/cnyes/newslist'), pageDelayMs: 0, useShowLog: false })
    ...
    it('targetTotal達標即停止翻頁', async function() {
        let c0 = svr.getCount('/cnyes/newslist')
        let t = await fetchCnyes({ url: svr.url('/cnyes/newslist'), targetTotal: 1, pageDelayMs: 0, useShowLog: false })
    ...
    it('HTTP 403納入重試範圍, 含初始共執行maxRetries+1次', async function() {
        ...
        await fetchCnyes({ url: svr.url('/status/403'), maxRetries: 2, baseDelayMs: 10, maxDelayMs: 10, pageDelayMs: 0, useShowLog: false })
            .catch(() => {})
