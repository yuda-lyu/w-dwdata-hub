...
let test = async () => {
    ...
    let r1 = await wi.fetchRSS('https://www.ptt.cc/atom/Gossiping.xml', { useShowLog: false })
    ...
    let r1b = await wi.fetchRSS('https://alphaarchitect.com/feed', { method: 'curl', withContent: true, useShowLog: false })
    console.log('rss(curl):', r1b.length, r1b[0].content.length, r1b[0].description.length)
    // => rss(curl): 5 6339 545

    //綜合新聞: Hacker News最新文章, 可指定取回篇數
    let r2 = await wi.fetchHackerNews(5, { useShowLog: false })
    ...
    let r3 = await wi.fetchAiNewsAggregator({ useShowLog: false })
    console.log('aiNews:', r3.length, r3[0].from)
    // => aiNews: 914 夕小瑶科技说

    //台股新聞: 鉅亨網, 逐頁抓至targetTotal則
    let r4 = await wi.fetchCnyes({ targetTotal: 30, useShowLog: false })
    ...
    let r5 = await wi.fetchTwseStock('20260806', '2330', { useShowLog: false })
    console.log('twseStock:', r5.stat, r5.data.length, r5.data[0][6])
    // => twseStock: OK 1 2,365.00

    //台股資料: 櫃買中心上櫃融資融券, 未指定個股則回全市場
    let r6 = await wi.fetchTpexMargin('20260806', ['6488'], { useShowLog: false })
    ...
    let r7 = await wi.fetchTaifex('20260806', { useShowLog: false })
    console.log('taifex:', r7.futures.tx.close, r7.pcRatio.ratio, r7.errors)
    // => taifex: 44280 109.14 []

    //台股資料: 證交所休市日期, 給定日期則另回傳該日是否休市
    let r8 = await wi.fetchTwDataHoliday('20260101', { useShowLog: false })
    ...
    let r9 = await wi.fetchAisixiang.fetchAuthorArticles({ name: '葛兆光', useShowLog: false })
    ...
    let r10 = await wi.fetchAisixiang.fetchArticle({ aid: '146669', useShowLog: false })
    ...
    let r11 = await wi.fetchGuancha.fetchTopicArticles({ name: '财经', maxPages: 1, useShowLog: false })
    ...
    let r12 = await wi.fetchGuancha.fetchAuthorArticles({ name: '不存在的作者XYZ', useShowLog: false })
